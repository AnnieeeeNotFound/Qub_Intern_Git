"""
Step 7: Two-layer feed-forward network (2-2-1) on the memristor chip.

Downscaled hardware replication of  MIF + Forward-Forward
...
Training is two greedy phases, mirroring train_next_layer:
...
try:
    ...
    plot_path = os.path.join(
        output_directory,
        f"ff2_{DATASET}_{MODE}_{TARGET_MODE}.png",
    )
    ...
    csv_path = os.path.join(
        output_directory,
        time.strftime(
            f"ff2_{DATASET}_{MODE}_%Y%m%d_%H%M%S.csv"
        ),
    )
